...
public struct PresetHeader {
    let presetName: FixedSizeArray<Byte>  // 20 characters, filled up with zeros
    let preset: Word
    let bank: Word
    ...
    public init(presetName: FixedSizeArray<Byte>, preset: Word, bank: Word, presetBagIndex: Word, library: DWord, genre: DWord, morphology: DWord) {
...
public struct InstrumentSubChunk {
    let instrumentName: FixedSizeArray<Byte>  // 20 characters, filled up with zeros
    let instrumentBagIndex: Word
    
    public init(instrumentName: FixedSizeArray<Byte>, instrumentBagIndex: Word) {
...
public struct SampleHeaderSubChunk {
    ...
    public init(instrumentName: FixedSizeArray<Byte>, start: DWord, end: DWord, startLoop: DWord, endLoop: DWord, sampleRate: DWord, originalPitch: Byte, pitchCorrection: Int8, sampleLink: Word, sampleType: SampleLinkType) {
        self.instrumentName = instrumentName
        self.start = start
        self.end = end
